	...
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if ((entity.getDirection()) == Direction.EAST) {
			if (world instanceof ServerLevel _serverworld) {
				StructureTemplate template = _serverworld.getStructureManager().getOrCreate(new ResourceLocation("db", "lightcorry22c"));
				if (template != null) {
					template.placeInWorld(_serverworld, new BlockPos(x, y - 2, z - 10), new BlockPos(x, y - 2, z - 10),
		...
		if ((entity.getDirection()) == Direction.NORTH) {
			if (world instanceof ServerLevel _serverworld) {
				StructureTemplate template = _serverworld.getStructureManager().getOrCreate(new ResourceLocation("db", "lightcorry22c"));
				if (template != null) {
					template.placeInWorld(_serverworld, new BlockPos(x - 10, y - 2, z), new BlockPos(x - 10, y - 2, z),
